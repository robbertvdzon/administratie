...
object ImportFromAbnAmroHelper {
    ...
    fun findNextAfschriftNummer(gebruiker: Gebruiker): Int {
        return 1 + (gebruiker.defaultAdministratie.afschriften.map({ afschrift -> Integer.parseInt(afschrift.nummer) }).maxWith(Comparator.naturalOrder<Int>()) ?: START_AFSCHRIFT_NUMMER)
    }
